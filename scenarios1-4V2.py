...
path = "/Users/lsjnancy/Desktop/"
name = input("Enter how you would like to name the file (without .rou.xml): ")
fileName = os.path.join(path, name+".rou.xml") 
f = open(fileName,"w")

f.write("<routes>\n")

f.write("<vType id=\"car\"\n     vClass=\"passenger\" length=\"7\" accel=\"4\" decel=\"2\"\n     sigma=\"1.0\" maxspeed=\"20\"/>\n")


# Initialize variables
time = 1.0
vehicleID = 1
s1_upperRange = int(Scenario_list[0]*Sim_Time/100)
s2_upperRange = int(s1_upperRange + Scenario_list[1]*Sim_Time/100)
s3_upperRange = int(s2_upperRange + Scenario_list[2]*Sim_Time/100)
s4_upperRange = int(s3_upperRange + Scenario_list[3]*Sim_Time/100)
# Generate a random number between 0 and 100, use this number to determine the scenario type for the vehicle departing at the next time step
# Repeat this process until the simulation time ends
# This assumes that total # of vehicles depart = total simulation time (in seconds)
while time <= Sim_Time:
    r = random.randint(1,Sim_Time)
    if r >= 1 and r <= s1_upperRange:
        while True:
            ...
            if len(route) != 0:
                break
        f.write("<vehicle id=\"{0}\" type=\"car\" depart=\"{1:.2f}\"> \n     <route edges=\"{2}\"/>\n</vehicle>\n".format(vehicleID,time,route))
        time = time + 1
        vehicleID = vehicleID + 1
    elif r >= s1_upperRange+1 and r <= s2_upperRange:
        while True:
        ...
        time = time + 1
        vehicleID = vehicleID + 1
    elif r >= s2_upperRange+1 and r <= s3_upperRange:
        while True:
            fromNode = generateOutCityNode()
            toNode = generateInCityNode()
            route = dijsktra(unweighted_graph,'{}'.format(fromNode),'{}'.format(toNode))
        ...
        time = time + 1
        vehicleID = vehicleID + 1
    elif r >= s3_upperRange + 1 and r <= s4_upperRange:
        while True:
            fromNode = generateInCityNode()
            toNode = generateOutCityNode()
            route = dijsktra(unweighted_graph,'{}'.format(fromNode),'{}'.format(toNode))
